...
const FROM    = process.env.EMAIL_FROM || 'GunVault <noreply@gunvault.com>';
...
async function sendWelcome(user) {
  await sendMail({
    to: user.email,
    subject: 'Welcome to GunVault — Your FFL Dashboard is Ready',
    html: `
      <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto">
        <h2 style="color:#c41e3a">Welcome to GunVault, ${user.name}!</h2>
        ...
        <p style="color:#999;font-size:12px">GunVault — Compliant FFL Management Software</p>
      </div>
    `
  });
}
...
async function sendPasswordReset(user, token) {
  ...
  await sendMail({
    to: user.email,
    subject: 'GunVault — Reset Your Password',
    html: `
      <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto">
        <h2 style="color:#c41e3a">Reset Your Password</h2>
        <p>Hi ${user.name}, we received a request to reset your GunVault password.</p>
...
async function sendDemoConfirm(demo) {
  await sendMail({
    to: demo.email,
    subject: 'GunVault Demo Request Received',
    html: `
      <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto">
